...
Harrison.prototype._getClient = function() {
    if (!this._client) {
        ...
        if (exports.AUTO_CLOSE) {
            this._client.addListener("idle", function() {
                var self = this;

                // TODO this probably belongs in the redis client as
                // `shutdown()` or somesuch (using `process.nextTick()` to shut
                // down cleanly)
...
Harrison.prototype._getDefinition = function(taskId, callback) {
    ...
    self._getClient().hgetall(self._namespace("tasks:" + taskId), x(function(reply) {
        ...
        // TODO this feels dirty and precludes binary args (which is probably
        // ok)
        for (p in reply) {
            reply[p] = reply[p].toString();
        }
