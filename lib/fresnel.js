...
Fresnel.prototype._runTask = function(task, callback) {
    ...
    if (endpoint) {
        var publicTask = this.getPublicDefinition(task);
        publicTask.retry = task.attempts > 0;
        this.POST(endpoint, JSON.stringify(publicTask), function(rsp, body, requestTime) {
            // 2xx => task completed
