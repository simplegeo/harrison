 ...
 */
Fresnel.prototype._addToReservoir = function(task, scheduledFor, callback) {
    this._getClient().zadd(this._namespace("reservoir"), scheduledFor, task.id, x(callback));
};
...
Fresnel.prototype._taskCompleted = function(callback, task, status, response) {
    ...
    if (status) {
        ...
        if (task.attempts >= exports.MAX_RETRIES) {
            ...
            self._addToReservoir(task, scheduledFor, onCompletion);
        }
...
Fresnel.prototype._queueTask = function(task, scheduledFor, callback) {
    ...
    if (scheduledFor.getTime() > new Date().getTime() + 60 * 1000) {
        this._addToReservoir(task, scheduledFor.getTime(), onCompletion);
    } else {
