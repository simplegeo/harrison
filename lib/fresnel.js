            ...
            self._getDefinitions(taskIds, function(taskDefs) {
                ...
                taskIds.forEach(function(taskId) {
                    ...
                    self._getClient().sadd(self._namespace("pending"), taskId, x(function(reply) {
                        ...
                        self._getClient().hset(self._namespace("tasks:" + taskId), "reservedAt", new Date().toISOString(), x(onCompletion));
                    }));
                });
            });
        } else {
...
Fresnel.prototype.createTask = function(task, callback) {
    ...
    self._isQueued(task, function(isQueued) {
        if (isQueued) {
            ...
            self._createDefinition(task, function(taskId) {
                self._queueTask(taskId, null, function() {
                    self._getClient().sadd(self._namespace("tasks"), self._hash(task), x(function() {
                        if (callback) {
                            callback(taskId);
                        }
                    }));
...
Fresnel.prototype._taskCompleted = function(callback, task, status, response) {
    ...
    if (status) {
        var onCompletion = callback && callback.barrier(3, [task, response]);
 ...
 */
Fresnel.prototype._queueTask = function(taskId, scheduledFor, callback) {
    var now = new Date();
    scheduledFor = scheduledFor || now;

    var onCompletion = callback && callback.barrier(4);

    this._getClient().hmset(this._namespace("tasks:" + taskId), "scheduledFor", scheduledFor.toISOString(), "queuedAt", now.toISOString(), x(onCompletion));
    this._getClient().hsetnx(this._namespace("tasks:" + taskId), "firstScheduledFor", scheduledFor.toISOString(), x(onCompletion));
    this._getClient().hsetnx(this._namespace("tasks:" + taskId), "firstQueuedAt", now.toISOString(), x(onCompletion));
    this._getClient().zadd(this._namespace("queue"), scheduledFor.getTime(), taskId, x(onCompletion));
}
