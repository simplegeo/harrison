...
Fresnel.prototype._taskCompleted = function(callback, task, status, response) {
    ...
    if (status) {
        var onCompletion = callback && callback.barrier(2, wrapper);
        ...
        var onCompletion = callback && callback.barrier(4, wrapper);
        ...
        self._getClient().zadd(self._namespace("failed"), ++task.attempts, task.id, x(onCompletion));
        self._getClient().set(self._namespace("errors:" + task.id), response, x(onCompletion));
        self._getClient().srem(self._namespace("pending"), task.id, x(onCompletion));
