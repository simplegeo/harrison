...
Fresnel.prototype._queueTask = function(taskId, timestamp, callback) {
    ...
    this._getClient().hset(this._namespace("tasks:" + taskId), "scheduledFor", timestamp.toISOString(), x(onCompletion));
    this._getClient().hset(this._namespace("tasks:" + taskId), "queuedAt", now.toISOString(), x(onCompletion));
    this._getClient().zadd(this._namespace("queue"), timestamp.getTime(), taskId, x(onCompletion));
}
