...
Fresnel.prototype._queueTask = function(taskId, timestamp, callback) {
    ...
    this._getClient().hset(this._namespace("tasks:" + taskId), "scheduledFor", timestamp, x(onCompletion));
    this._getClient().hset(this._namespace("tasks:" + taskId), "queuedAt", now, x(onCompletion));
    this._getClient().zadd(this._namespace("queue"), timestamp, taskId, x(onCompletion));
}
